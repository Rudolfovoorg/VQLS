...
def HE_ansatz(n: int, d: int, params: list) -> QuantumCircuit:
    """
    Creates a hardware-efficient ansatz with rotational and entangling layers.
...
def fixed_ansatz(n: int, d: int, params: list) -> QuantumCircuit:
    """
    Creates a lighter ansatz with RY rotations and CZ entangling layers.
    ...
    for i in range(d):
        ...
        # Apply nearest-neighbor entangling CZ gates
        for j in range(0, n - 1, 2):
            qc.cz(j, j + 1)
        for j in range(1, n - 1, 2):
            qc.cz(j, j + 1)
