...
class HadamardTest:
    """Statevector-based Hadamard test for estimating <psi| W |psi>."""

    def __init__(self, U_psi, Us):
        """
        Parameters:
        U_psi: The unitary preparing the target state |psi> = U_psi|0>.
        Us (list): A list of unitaries whose product W is measured.
        """
        self.U_psi = U_psi
        self.Us = Us
        ...
        if not all(U.num_qubits == U_psi.num_qubits for U in Us):
            raise TypeError("Us must have the same number of qubits as U_psi")
...
class SamplerHadamardTest(HadamardTest):
    """Shot-based Hadamard test using the Aer sampler."""
    ...
    def _run_circuit(self, qc_transpiled):
        sampler = Sampler()
        return sampler.run([qc_transpiled], shots=self.num_shots).result()
    ...
    def get_expectation_real(self):
        ...
        return (counts.get('0', 0) - counts.get('1', 0)) / self.num_shots
